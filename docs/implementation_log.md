...
Continued assimilating real-laminography report schema behavior by moving the
per-view 5-DOF pose-parameter summary out of the reference-regression script and
into the bench-owned report module.

Changes:

- Added `real_lamino_pose_params_summary(...)` to
  `tomojax.bench.real_laminography_report`.
- Exported the helper through the `tomojax.bench` facade.
- Updated the reference-regression runner, staged runner, and Pallas probe to
  use the bench-owned summary helper.
- Changed `real_lamino_safe_params_summary(...)` to use the bench-owned
  pose-summary schema by default.
- Added focused report/facade tests for the shared radians/degrees summary
  contract.
...
- `uv run pytest tests/test_real_laminography_report.py
  tests/test_public_facades.py::test_bench_facade_exports_developer_benchmark_helpers
  -q` passed: 7 tests in 2.90 seconds.
- `python tools/check_public_imports.py` passed.
- `git diff --check` passed.
...
Continued thinning `scripts/real_laminography/run_real_lamino_staged.py` by
moving finite-safe pose-parameter summary behavior into the bench-owned report
module.

Changes:

- Added `real_lamino_safe_params_summary(...)` to
  `tomojax.bench.real_laminography_report`.
- Exported the helper through the `tomojax.bench` facade.
- Updated the staged runner to use the bench-owned helper when recording
  failed pose-stage provenance.
- Added focused report/facade tests that verify non-finite parameter arrays are
  not summarized and finite arrays are normalized before summary.
...
## 2026-05-17 - Real-laminography planned-stage manifests moved behind bench facade

### Scope

Continued thinning `scripts/real_laminography/run_real_lamino_staged.py` by
moving planned-stage manifest writing into the bench-owned report module.

Changes:

- Added `write_real_lamino_planned_stage_manifests(...)` to
  `tomojax.bench.real_laminography_report`.
- Exported the helper through the `tomojax.bench` facade.
- Updated the staged runner to use the bench-owned helper.
- Added focused report/facade tests that verify planned manifests are written
  for planned stages and existing stage manifests are not overwritten.
...
- `uv run pytest tests/test_real_laminography_report.py
  tests/test_public_facades.py::test_bench_facade_exports_developer_benchmark_helpers
  -q` passed: 5 tests in 2.11 seconds.
- `python tools/check_public_imports.py` passed.
- `git diff --check` passed.
...
Continued thinning `scripts/real_laminography/run_real_lamino_staged.py` by
moving fail-closed stage artifact writing into the bench-owned report module.

Changes:

- Added `mark_real_lamino_stage_failed(...)` and
  `write_real_lamino_skipped_stage_manifests(...)` to
  `tomojax.bench.real_laminography_report`.
- Exported both helpers through the `tomojax.bench` facade.
- Updated the staged runner to use the bench-owned report helpers directly.
- Added focused tests for failure provenance persistence and skipped-stage
  manifest writing without overwriting existing stage evidence.
...
- `uv run pytest tests/test_real_laminography_report.py
  tests/test_public_facades.py::test_bench_facade_exports_developer_benchmark_helpers
  -q` passed: 3 tests in 1.84 seconds.
- `python tools/check_public_imports.py` passed.
- `git diff --check` passed.
