# Real Laminography v2 Staged Production Baseline - 2026-05-12

## Status

This is the morning-facing productionization baseline for the current TomoJAX v2
real-laminography staged workflow. It promotes the completed k11 reference run
as the strongest current evidence, while keeping the distinction between the
clean `staged-lamino` profile and the strict internal `reference-regression`
path.
...
a real-data demo candidate. This does not prove general production readiness
...
The production-named staged profile now exists. The clean command is:
...
env LD_LIBRARY_PATH="$NVLIB${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
  ...
  uv run python scripts/real_laminography/run_real_lamino_staged.py \
    ...
    --out runs/real_lamino_v2_staged_lamino_k11_54014_20260512 \
...
The strict regression command is the same except for
`--profile reference-regression`. That profile preserves the internal
reference-run comparison contract and should stay separate from the user-facing
staged-lamino profile.

## Metrics

| Quantity | v1 reference | v2 reference-regression run |
| --- | ---: | ---: |
...
The public-facing `staged-lamino` profile should present this as baseline,
...
## Reference-Regression Audit

The strict reference-regression contract passed: schedules, bounds, reconstruction
settings, pose model, preprocessing, and final-candidate policy matched the
expected v1-derived contract.

Reference-regression table status counts:
...
Follow-up code now makes strict `reference-regression` reruns replay the
reference setup stage row counts from the reference `stage_summary.csv` files.
That keeps the ordinary `staged-lamino` profile unchanged while making the
regression structure honest against the reference stage/level/iteration
contract.
...
staged-lamino result with honest provenance.

## Original Plan Status

| Phase | Current status | Notes |
| --- | --- | --- |
| Phase 0 baseline and benchmark contract | `partial` | Real k11 report and reference artifacts exist; broad benchmark contract is still incomplete. |
| Phase 1 geometry graph, parameters, gauges | `implemented_for_staged_lamino` | Setup and pose state, bounds, gauge-like mean translation handling, and JSON/CSV artifacts exist for this path. Full general geometry graph is still partial. |
| Phase 2 JAX reference forward model and residual loss | `implemented_for_staged_lamino` | Tomography/laminography projectors and level losses are working in the real run. Robust pseudo-Huber default remains broader-plan work. |
| Phase 3 FISTA / Huber-TV preview reconstruction | `implemented_for_staged_lamino` | Huber-TV/FISTA and measured-L fallback are required for the winning run. |
| Phase 4 pose-only 5x5 LM | `implemented_for_staged_lamino` | Phi, dx/dz, and 5DOF polish run and improve the final result in the reference-regression path. |
| Phase 5 setup-only LM | `implemented_for_staged_lamino` | COR/det_u, detector roll, and axis direction stages run on the reference data. |
| Phase 6 joint setup+pose Schur LM | `partial` | Synthetic Schur diagnostics exist, but the real staged-lamino path is still staged rather than a single default joint Schur solve. |
| Phase 7 alternating solver and continuation | `partial` | Alternating infrastructure and artifacts exist; staged-lamino uses a v1-like staged continuation. |
| Phase 8 nuisance and weak DOF handling | `diagnostic_only` | Nuisance/weak-mode work is not a staged-lamino success claim. |
| Phase 9 Pallas fast paths | `partial` | Pallas fast paths exist and rigid detector-grid folding was added, but reference-regression mode intentionally preserves measured-FISTA fallback where required. |
| Phase 10 experimental modules | `out_of_scope_for_staged_lamino` | Not needed for the overnight real-data staged run. |
...
- The overnight staged-lamino run does not require the hard
...
- The phi level-2 loss explosion was rooted to the calibrated-grid Huber-FISTA
  core fallback and fixed in the reference-regression path by restoring
  measured-L public FISTA.
- Fail-closed checks prevent non-finite pose-stage volumes from being promoted.

## What Remains Unproven Or Messy

- A clean `staged-lamino` profile now exists and resolves to the winning
  reference-derived settings, but it has not been rerun under a
  production-named output directory.
- `reference-regression` remains available and should stay separate from the
  user-facing staged profile.
- `run_real_lamino_staged.py` still mixes profile selection, execution,
  validation, reporting, and reference table generation.
- Stage contact sheets and v1-vs-v2 final comparison sheets are now committed
  ...
  automatically.
- Synthetic tomography production gates for `synth128_setup_global_tomo` and
...
1. Split the real-runner profile/report/reference-regression responsibilities
   enough that the working staged-lamino path is not buried as an accidental
   flag combination.
2. Add focused tests for phi level-2 loss scale, final
   candidate selection, fail-closed output, and portable artifact paths.
3. Fix the synthetic tomography production-gate blockers exposed by the bounded
   report: setup-global axis/roll recovery and pose-random dx/dz/phi
   evaluation.
