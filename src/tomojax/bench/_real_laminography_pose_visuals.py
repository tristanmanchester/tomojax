"""TEM-grid pose visualization artifacts for real-laminography reports."""
...
def _read_pose_params(path: Path) -> dict[str, np.ndarray]:
...
def render_tem_grid_pose_artifacts(
    *,
    params_path: Path,
    out_dir: Path,
    radius: float = 80.0,
    ...
    _draw_static_pose_png(geom, png_path, yaw_deg=float(yaw_deg), pitch_deg=float(pitch_deg))
...
__all__ = ["render_tem_grid_pose_artifacts"]
