...
class PoseLbfgsLoopResult:
    ...
    failure_initial_loss: float | None = None
...
def _build_pose_lbfgs_transform(
    *,
    ...
    motion_model: PoseMotionModel | None,
    smooth_unbounded_coefficients: bool,
) -> PoseLbfgsTransform:
    if smooth_unbounded_coefficients:
        ...
        z0 = jnp.asarray(motion_coeffs_in, dtype=jnp.float32)
...
def _run_pose_lbfgs_optax_loop(
    *,
    ...
    def record_value(value: float, z_value: jnp.ndarray) -> None:
    ...
    if not math.isfinite(initial_value) or not math.isfinite(initial_grad_norm):
        return PoseLbfgsLoopResult(
            ...
            success=False,
            message="initial L-BFGS objective/gradient is non-finite",
            nit=0,
            last_grad_norm=initial_grad_norm,
            last_step_norm=0.0,
            failure_message="initial L-BFGS objective/gradient is non-finite",
            failure_initial_loss=initial_value if math.isfinite(initial_value) else None,
        )
    ...
    try:
        for iteration in range(int(cfg.maxiter)):
            ...
            if not math.isfinite(value_f) or not math.isfinite(grad_norm):
                return PoseLbfgsLoopResult(
                    ...
                    initial_grad_norm=initial_grad_norm,
                    success=False,
                    message="non-finite L-BFGS objective/gradient",
                    nit=nit,
                    last_grad_norm=last_grad_norm,
                    last_step_norm=last_step_norm,
                    failure_message="non-finite L-BFGS objective/gradient",
                    failure_initial_loss=initial_value,
                )
            record_value(value_f, z)
            if grad_norm <= float(cfg.gtol):
                success = True
                message = f"Optax L-BFGS gradient norm {grad_norm:.3e} <= gtol"
            ...
            if not bool(jnp.all(jnp.isfinite(z_next))):
                return PoseLbfgsLoopResult(
                    ...
                    initial_grad_norm=initial_grad_norm,
                    success=False,
                    message="non-finite L-BFGS parameter update",
                    nit=nit,
                    last_grad_norm=last_grad_norm,
                    last_step_norm=last_step_norm,
                    failure_message="non-finite L-BFGS parameter update",
                    failure_initial_loss=initial_value,
                )
            ...
            if line_search_value is not None:
                ...
                if math.isfinite(line_value_f):
                    record_value(line_value_f, z_next)
        ...
        if is_expected_failure(exc):
            return PoseLbfgsLoopResult(
                ...
                initial_grad_norm=initial_grad_norm,
                success=False,
                message=f"L-BFGS failed during optimization: {exc}",
                nit=nit,
                last_grad_norm=last_grad_norm,
                last_step_norm=last_step_norm,
                failure_message=f"L-BFGS failed during optimization: {exc}",
                failure_initial_loss=initial_value,
            )
        raise
...
def run_pose_lbfgs(
    *,
    ...
    def _objective(z_candidate: jnp.ndarray) -> jnp.ndarray:
        candidate, _ = transform.params_from_z(z_candidate)
    ...
    if loop.failure_message is not None:
        return _failure_result(
            loop.failure_message,
            initial_loss=loop.failure_initial_loss,
            eval_count=loop.eval_count,
            best_value=loop.best_value,
        )
    initial_value = float(loop.initial_value)
    initial_grad_norm = float(loop.initial_grad_norm)
    best_value = loop.best_value
    eval_count = int(loop.eval_count)
    best_z = loop.best_z
    ...
    def _evaluate_candidate(
        ...
        candidate_params, candidate_coeffs = transform.params_from_z(z_candidate)
    ...
    candidates = [_evaluate_candidate("last", loop.z)]
    ...
    if math.isfinite(final_loss):
        eval_count += 1
        if final_loss < best_value:
            best_value = float(final_loss)
    ...
    stats: OptimizerStats = {
        "lbfgs_backend": "optax",
        "lbfgs_success": bool(loop.success),
        "lbfgs_accepted": bool(accepted),
        "lbfgs_fallback_to_gd": False,
        "lbfgs_status": 0 if loop.success else 1,
        "lbfgs_message": result_message,
        "lbfgs_nit": int(loop.nit),
        "lbfgs_nfev": int(eval_count),
        "lbfgs_objective_evals": int(eval_count),
        "lbfgs_line_search_steps": int(loop.total_line_search_steps),
        ...
        "lbfgs_final_grad_norm": float(loop.last_grad_norm),
        "lbfgs_last_step_norm": float(loop.last_step_norm),
    }
