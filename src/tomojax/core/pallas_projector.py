...
def _cached_grad_pallas_call(
    *,
    ...
    unroll: int | None,
    interpret: bool,
) -> Callable[[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray], jnp.ndarray]:
    kernel = functools.partial(
        _projector_grad_kernel,
        nx=int(nx),
        ny=int(ny),
        nz=int(nz),
        nu=int(nu),
    ...
    return pl.pallas_call(
        ...
        interpret=bool(interpret),
        compiler_params=plt.CompilerParams(num_warps=int(num_warps)),
        name="tomojax_forward_grad_T_pallas",
    )
...
def forward_project_loss_and_grad_T_pallas(
    ...
    *,
    ...
    if not compute_loss:
        grad_flat = _cached_grad_pallas_call(**common_call_kwargs)(*call_args)
        return jnp.asarray(0.0, dtype=jnp.float32), grad_flat.reshape((int(nx), int(ny), int(nz)))

    call = _cached_loss_grad_pallas_call(
        **common_call_kwargs,
        compute_loss=True,
    )
    partial_loss, grad_flat = call(*call_args)
